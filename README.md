...
- **Journey is a map.** The seven days wind from the last lesson to a black "Next lesson with Melisande" box: a note a day (filled when done, ring on today), a token beside each day, +2 at the end, and a red mark on the day anything was flagged. Below the map, in order: "At the lesson: show Melisande" (the checklist, where the path ends) and "At the lesson: ask Melisande" (the flags, in a red-edged box), then the tokens
- **Stages to the goal are the teacher's.** The Goal tab's Foundations / Repertoire / Polish / Mock / Exam shape is shown as suggested until the teacher types her own stages and weeks in the pupil's profile
