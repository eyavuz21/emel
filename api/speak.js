...
// Teacher only. The file goes into the public melodigo-audio bucket under the studio's folder, using the
...
export default async function handler(req, res) {
  ...
  const up = await fetch(`${url}/storage/v1/object/melodigo-audio/${safePath}`, {
  ...
  return res.status(200).json({ url: `${url}/storage/v1/object/public/melodigo-audio/${safePath}`, chars: clean.length });
}
