...
function adminClient() {
  ...
  return {
    async all() { const r = await fetch(`${url}/rest/v1/sostenuto_students?select=user_id,data`, { headers: h }); if (!r.ok) throw new Error(`students ${r.status}`); return r.json(); },
    async one(id) { const r = await fetch(`${url}/rest/v1/sostenuto_students?select=user_id,data&user_id=eq.${id}`, { headers: h }); const j = await r.json(); return j[0] || null; },
    async save(id, data) { const r = await fetch(`${url}/rest/v1/sostenuto_students?user_id=eq.${id}`, { method: "PATCH", headers: { ...h, prefer: "return=minimal" }, body: JSON.stringify({ data, updated_at: new Date().toISOString() }) }); if (!r.ok) throw new Error(`save ${r.status}`); },
  };
}
